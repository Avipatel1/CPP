...
the following formula: distance fallen = gravity_constant * x_seconds^2 / 2
...
void printResult(double height, double time);
...
int main()
{
    ...
    std::cout << "Enter length of time to be used for calculation (seconds): ";
    double time{};
    std::cin >> time;
    
    printResult(height, time);
...
void printResult(double height, double time)
{
    ...
    {
        if ((height - kGravity * (i*i) / 2) <= 0)
        {
            std::cout << "At " << i << " seconds, the ball is on the ground" << '\n';
            std::cout << "Calculation aborted";
            break;
        }
        std::cout << "At " << i << " seconds, the ball is at height: " << height - kGravity * (i*i) / 2 << '\n';
    }
}